...
def parse_arguments():
    ...
    parser.add_argument('--seed', default=2024, type=int)  # default=None
    parser.add_argument('--workers', default=8, type=int)
    ############
    parser.add_argument('--supp_idx', default=0, type=int)
    parser.add_argument('--test_label', type=int, nargs='+', default=[1, 2, 3, 6], help='A list of numbers')  # SABS
    # parser.add_argument('--test_label', type=int, nargs='+', default=[1, 2, 3, 4], help='A list of numbers')  # chaost
    # parser.add_argument('--test_label', type=int, nargs='+', default=[1, 2, 3], help='A list of numbers')  # cmr
...
def main():
    ...
    # Set up logging.
    logger = set_logger(args.save_root, 'logger.log')
    logger.info(args)
    ...
    for label_val, label_name in labels.items():
        ...
        if label_name == 'BG':
            continue
        elif (not np.intersect1d([label_val], args.test_label)):  # If label_val is not in the list of _config ['test'label '], execute the following code block
            continue
        ...
        if args.EP1 is True:
            ...
            for sub_chunck in range(args.n_shot):
                ...
                for i in range(query_image_s.shape[0]):
                    query_pred_t = model([support_image_s], [support_fg_mask_s], [query_image_s[[i]]],
                                       train=False)  # 1 x 2 x H x W
                    query_pred_ss.append(query_pred_t)
                query_pred_ss = torch.cat(query_pred_ss, dim=0)
                query_pred_s = query_pred_ss.argmax(dim=1).cpu()  # C x H x W
            ...
            query_pred_s = []
            for i in range(query_image_s.shape[0]):
                query_pred = model([support_image], [support_fg_mask], [query_image_s[[i]]],
                                         train=False)  # 1 x 2 x H x W
                query_pred_s.append(query_pred)
            query_pred_s = torch.cat(query_pred_s, dim=0)
            query_pred = query_pred_s.argmax(dim=1).cpu()
